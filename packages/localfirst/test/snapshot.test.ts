...
const todo = (n: number) => ({
  id: new Uuid(BigInt(n)),
  title: `t${n}`,
  done: n % 2 === 0,
  createdAt: new Timestamp(BigInt(n)),
});
...
describe('SnapshotStore', () => {
  it('round-trips rows and alternates slots with increasing generations', async () => {
    ...
    await snap.save(
      new Map([
        ['todos', [todo(1), todo(2)]],
        ['counters', [{ name: 'a', value: 5n }]],
      ]),
      { serverTsMicros: 10n, workingSetHash: 'h' }
    );
    await snap.save(
      new Map([
        ['todos', [todo(3)]],
      ...
      { serverTsMicros: 20n, workingSetHash: 'h' }
    );
  ...
  it('falls back to the older slot when the newer one is torn', async () => {
    ...
    await snap.save(
      ...
      { serverTsMicros: 1n, workingSetHash: 'h' }
    );
    await snap.save(
      new Map([
        ['todos', [todo(2)]],
      ...
      { serverTsMicros: 2n, workingSetHash: 'h' }
    );
