 ...
 *     provides; the plugin aliases it to `stdb-localfirst/sys-shim`;
...
export function patchServerEntry(source: string): string {
  if (!source.includes(WINDOW_NEEDLE)) {
    throw new Error(
      'stdb-localfirst: spacetimedb/server polyfill changed; update the bundler plugin'
    );
  }
...
/** esbuild plugin: `plugins: [localfirstEsbuildPlugin()]`. */
export function localfirstEsbuildPlugin(): {
  name: string;
  setup(build: EsbuildPluginBuild): void;
} {
  return {
    name: 'stdb-localfirst',
    setup(build) {
...
/** Vite / Rollup plugin: `plugins: [localfirstVitePlugin()]`. */
export function localfirstVitePlugin(): {
  name: string;
  enforce: 'pre';
