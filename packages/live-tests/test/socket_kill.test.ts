...
import { attach, connect, drained, localView, mod, openLocal, serverView, sleep } from '../src/client';
...
describe('socket killed mid-flight', () => {
  it('client-side disconnect right after sending: exactly-once on the server', async () => {
    ...
    for (let r = 0; r < rounds; r++) {
      ...
      const conn = await connect({ wsUrl: server.wsUrl, db: 'todo-lf', token, onDisconnect: () => c.lf.disconnect() });
      token = conn.token;
      ...
      const again = await connect({ wsUrl: server.wsUrl, db: 'todo-lf', token, onDisconnect: () => c.lf.disconnect() });
  ...
  it('server SIGKILLed mid-flight, restarted on the same port: client converges after reconnect', async () => {
    const c = await openLocal({ inflightWindow: 4 });
    const handles = [1, 2, 3, 4].map(i => c.lf.call(mod.bump, { name: `crash${i}`, by: 10n }));
    await Promise.all(handles.map(h => h.durable));

    const first = await connect({ wsUrl: server.wsUrl, db: 'todo-lf', onDisconnect: () => c.lf.disconnect() });
    ...
    const again = await connect({ wsUrl: server.wsUrl, db: 'todo-lf', token: first.token, onDisconnect: () => c.lf.disconnect() });
    attach(c.lf, again.conn);
    for (const h of handles) expect(await h.settled).toBe('acked');
    await drained(c.lf);
    await sleep(100);
    expect(localView(c.lf)).toEqual(serverView(again.conn));
    for (let i = 1; i <= 4; i++) {
      expect((c.lf.db.counters.name.find(`crash${i}`) as any).value).toBe(10n);
    }
    again.conn.disconnect();
    await c.cleanup();
  });
