...
import type { ReducerBinding } from '../client/worldline';
 ...
 * mounted worldline submodule tables, and the *real* wrapped reducer exports
...
export class FakeServer {
  ...
  sessionEpoch(clientId: Uuid, alias = 'wl'): bigint | null {
  ...
  isApplied(intentId: Uuid): boolean {
    const key = specKey(this.store.spec('wl.appliedIntents'));
