...
import { Worldline, type AnyReducer, type CallHandle, type IntentEvent } from '../client/worldline';
import { idKey } from '../client/intent_log';
import { WorldlineError } from '../client/errors';
...
import * as worldline from '../server/index';
...
class Simulation {
  ...
  constructor(opts: SimOptions) {
    ...
    this.server = new FakeServer(this.mod, this.bindings, { wl: worldline }, this.clock, serverRng);
  ...
  tryOpen(client: SimClient): Promise<Worldline> {
    return Worldline.open({
      module: this.mod,
  ...
  actionForeign(): void {
    ...
    ProductType.makeSerializer(binding.paramsType)(writer, {
      ...
      wlClient: this.foreignSession.clientId,
      wlEpoch: this.foreignSession.epoch,
    });
  ...
  async actionDoubleOpen(client: SimClient): Promise<void> {
    try {
      ...
      if (!(error instanceof WorldlineError)) {
