...
import { WorldlineError } from './errors';
...
/** Generated `reducers` maps nest submodule reducers under their alias (`wl.beginSession`). */
...
export interface WorldlineOptions {
   ...
   */
  beforeDrain?: (lf: Worldline) => Promise<void> | void;
  /** Alias the worldline submodule is mounted under in `schema()`. Default `wl`. */
...
export class Worldline {
  ...
  private constructor(
    ...
    const alias = options.submoduleAlias ?? 'wl';
  ...
  #registerReducers(options: WorldlineOptions): void {
    for (const [key, value] of Object.entries(options.module)) {
      ...
      const wrapped = value[WL_WRAPPED] === true;
      ...
      const entry: ReducerEntry = {
        ...
        name: binding.name,
        wrapped,
        innerFn: wrapped ? value[WL_INNER] : value,
  ...
  static async open(options: WorldlineOptions): Promise<Worldline> {
    const specs = options.tables ?? tableSpecsFromSchema(options.module.default);
    if (specs.length === 0)
      throw new WorldlineError(
    ...
    const lf = new Worldline(options, store, log, snapshot, releaseLock);
  ...
  #emit(event: IntentEvent): void {
    for (const listener of this.#listeners) {
      try {
        ...
        console.error('worldline: intent listener threw', error);
      }
    }
  }
  ...
  call(reducer: AnyReducer, args: Row = {}, options: CallOptions = {}): CallHandle {
    assert(!this.#closed, 'call() on a closed Worldline');
    ...
    if (bytes.length > INTENT_ARGS_BYTES_MAX) {
      throw new WorldlineError(`reducer arguments exceed ${INTENT_ARGS_BYTES_MAX} bytes`);
    }
    return bytes;
  }
  ...
  connect(link: Link): void {
    assert(!this.#closed, 'connect() on a closed Worldline');
  ...
  #runBeforeDrain(link: Link): void {
    ...
    Promise.resolve()
      ...
      .then(
        ...
        error =>
          console.warn('worldline: beforeDrain failed; not sending until next connect', error)
      );
  }
      ...
      .then(
        ...
        (error: unknown) => {
          ...
          if (!rotate && isIdentityMismatch(error)) {
            console.warn('worldline: identity changed; rotating the client id');
            this.#beginSession(link, generation, true);
            return;
          }
          console.warn('worldline: session handshake failed; not sending', error);
        }
      );
  }
  ...
  #onAcked(record: IntentRecord): void {
    ...
    this.log
      ...
      .catch(error => console.warn('worldline: could not persist ack', error));
  ...
  #afterSettled(): void {
    ...
    if (this.log.pending.size === 0 && this.log.marksSinceCompact >= every) {
      this.log.compact().catch(error => console.warn('worldline: compaction failed', error));
    }
    this.#drain();
  }
  ...
  #scheduleSnapshot(): void {
    ...
    this.#snapshotTimer = setTimer(() => {
      ...
      this.snapshotNow().catch(error => console.warn('worldline: snapshot failed', error));
    }, delayMs);
  }

  /** Write the base layer to disk now. */
  async snapshotNow(): Promise<SnapshotMeta> {
    assert(!this.#closed, 'snapshotNow() on a closed Worldline');
...
async function acquireLock(storage: StorageAdapter): Promise<(() => Promise<void>) | null> {
  ...
  if (release === null) {
    throw new WorldlineError(
      'storage is locked by another Worldline instance (another tab or process)'
    );
  }
  return release;
}
