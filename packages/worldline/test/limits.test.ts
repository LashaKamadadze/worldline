...
import { WorldlineError } from '../src/client/errors';
...
describe('limits', () => {
  it('refuses the intent past INTENTS_PENDING_MAX and stays consistent', async () => {
    ...
    expect(() => lf.call(mod.bump, { name: 'a', by: 1n })).toThrow(WorldlineError);
    expect(lf.pending().length).toBe(INTENTS_PENDING_MAX);
  ...
  it('refuses arguments larger than INTENT_ARGS_BYTES_MAX', async () => {
    ...
    expect(() => lf.call(mod.createTodo, { id: new Uuid(1n), title })).toThrow(WorldlineError);
