...
describe('deterministic simulation', () => {
  it('is deterministic for a given seed', async () => {
    const a = await runSimulation({
      seed: 1234,
      steps: 80,
      faults: { appendTorn: 0.05 },
      network: { dropAck: 0.1 },
    });
    const b = await runSimulation({
      seed: 1234,
      steps: 80,
      faults: { appendTorn: 0.05 },
      network: { dropAck: 0.1 },
    ...
    for (let seed = 1; seed <= SEEDS; seed++) {
      const r = await runSimulation({
        ...
        network: {
          dropAck: 0.15,
          dropCall: 0.05,
          minLatencyMicros: 500n,
          maxLatencyMicros: 20_000n,
        },
      });
      totals = {
        ...
        openRetries: totals.openRetries + r.openRetries,
      };
      if (r.violations.length)
        failures.push(`seed ${seed}: ${r.violations.slice(0, 5).join(' | ')}`);
    }
