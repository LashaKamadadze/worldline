...
import { WL_INNER, WL_WRAPPED } from '../src/shared/symbols';
...
describe('executor', () => {
  it('runs the inner body of an offline reducer against the store', () => {
    ...
    const inner = (mod.createTodo as any)[WL_INNER];
    expect((mod.createTodo as any)[WL_WRAPPED]).toBe(true);
  ...
  it('reports a reducer throw as failed and leaves nothing staged', () => {
    ...
    const out = executeReducer(
      store,
      (mod.createTodo as any)[WL_INNER],
  ...
  it('runs the wrapped export with dedup against namespaced submodule tables', () => {
    ...
    expect(store.count('wl.appliedIntents')).toBe(1);
  ...
  it('rejects an intent whose session is unknown, stale, or ahead of the server', () => {
    ...
    const call = (epoch: bigint, wlClient = clientId, intentId = new Uuid(7n)) =>
      executeReducer(
        store,
        mod.bump as any,
        {
  ...
  it('begin_session moves epochs forward and binds a client id to its first identity', () => {
    ...
    const attempt = (epoch: bigint, sender = new Identity(7n)) =>
      executeReducer(
        store,
        (ctx, args) => beginSessionBody(ctx.db.wl, ctx, args as any),
        { clientId, epoch },
        { ...info(), sender }
      );
    ...
    expect(store.count('wl.sessions')).toBe(1);
  });
});
...
function authoritativeStore(): LocalStore {
  const specs = [
    ...
    ...tableSpecsFromSchema(worldline.default, 'wl'),
  ];
