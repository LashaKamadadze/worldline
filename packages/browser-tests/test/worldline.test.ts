/**
 * Full Worldline inside real browsers against a real local SpacetimeDB:
...
describe('Worldline in the browser', () => {
  ...
  for (const name of ['chromium', 'firefox', 'webkit'] as const) {
    describe(name, () => {
      ...
      it('offline calls survive a reload and converge with the server once online', async ({
        ...
        if (!browser) return;
        const b = { browser };
        const dir = `wl-${name}-${randomUUID().slice(0, 8)}`;
        ...
        await context.setOffline(false);
        const identity = await t((t, ws) => t.connect(ws, 'todo-wl-browser'), stdb.wsUrl);
      ...
      it(conflictTitle, async ({ skip }) => {
        ...
        await ot((t, ws) => t.connect(ws, 'todo-wl-browser'), stdb.wsUrl);
        ...
        await context.setOffline(false);
        await t((t, ws) => t.connect(ws, 'todo-wl-browser'), stdb.wsUrl);
        await t(t => t.waitDrained(30_000));
        const settled = await t(t => t.settled());
